...
// This file is loaded on ALL 21 website pages.
// ====================================================================
...
// --- V. INITIALIZATION AND AUTHENTICATION CHECK (Self-Executing) ---
// This function runs immediately on load to set up the navigation bar based on the user's role.

(async function checkAuthentication() {
    // 1. Define all dynamic menu buttons
    const buttons = [
    ...
    try {
        ...
        console.warn('Auth check failed. User is likely unauthenticated.', error);
    }
    
    // 2. Show appropriate buttons based on role
    const showButton = (id) => document.getElementById(id)?.classList.remove('hidden');

    if (userRole === 'teacher') {
        showButton('logoutButton');
        showButton('mypageButton');
        showButton('validationButton');
        showButton('forumButton');
    } else if (userRole === 'admin') {
        showButton('logoutButton');
        showButton('validationButton');
        showButton('forumButton');
        showButton('createButton');
        showButton('adminButton');
    } else {
        // Default: show login if unauthenticated or error
